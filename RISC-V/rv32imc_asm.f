...
[UNDEFINED] Assembler   Vocabulary Assembler [THEN]
...
: ?$,           ( to opcode/reg typ -- )
  <$> @ IF   $links @ #31 > -reset? ?abort" Too much labels "
             ...
             herep  over 2 cells +  !           \ save adress
             over 3 < IF 4 ELSE 2 THEN allotp \ reserve space
             over 0=  IF 4 allotp THEN      \ + space for LI,
             tuck c!                                   \ Type
             tuck cell+ !                        \ Opcode/reg
             1 + c!                                      \ to
             0 <$> !                            \ delete flag
        ELSE 2>r  herep swap  2r>    \ -- from to opcode type
             dup 3 < IF 4 ELSE 2 THEN allotp     \ freihalten
             dup 0=  IF 4 allotp THEN     \ li, => lui, addi,
             offset,
        THEN ;
...
: R-Type:       ( opcode -- ; -- ) \ rd , rs1 , rs2
  ...
  Doesp>  >r  $f00c0300 $10040100 ?reg? 
          r> @p  r3@ +rd  r2@ +rs1  r1@  +rs2  ,p   reset ;
...
: I-Type1:      ( opcode -- ; -- ) \ rd ,( imm12 rs1
  ...
  Doesp>  >r  $fffc0300 $80040300 ?reg?   ?imm12?
          r> @p  r2@ +rd  r1@ +rs1  pars @ +imm12<<20  ,p   reset ;
...
: I-Type2:      ( opcode -- ; -- ) \ rd , rs1 , imm12
  ...
  Doesp>  >r  $f00c0300 $10040200 ?reg?   ?imm12?
          r> @p  r3@ +rd  r2@ +rs1  pars @ +imm12<<20  ,p   reset ;
...
: I-Type3:      ( opcode -- ; -- ) \ rd , rs1 , imm5
  ...
  Doesp>  >r  $f00c0300 $10040200 ?reg?   ?uimm5?
          r> @p  r3@ +rd  r2@ +rs1  pars @ +uimm5<<20  ,p   reset ;
...
: S-Type:       ( opcode -- ; -- ) \ rs2 ,( imm12 rs1
  Createp ,p
  Doesp>  >r  $fffc0300 $80040300 ?reg?   ?imm12?
          r> @p  r2@ +rs2  r1@ +rs1  pars @ +imm7|5  ,p   reset ;
...
: U-Type:       ( opcode -- ; -- ) \ rd , imm20
  ...
  Doesp>  >r  $fffc0300 $00040200 ?reg?   ?uimm20?
          r> @p  r2@ +rd  pars @ +uimm20<<12  ,p   reset ;
...
\ here cell+ ,p  here cell+ ,p  \ link/cfa if indirect threaded
  [ Assembler ] ready [ Forth ] also Assembler ;

: Code          ( name ; -- -1 ; -- ) \ actual no CFA
  Header  -1
\ herep 1 and IF 1 allotp THEN  \ !!! for test align address
\ here cell+ ,p \ link / cfa if indirect threaded
  [ also Assembler ] ready [ previous ] also Assembler ;
...
: End-Proc      ( -1 -- ; -- )  \ End of Proc or Code
  ...
  [ Assembler ] check [ Forth ]  previous  End-Create ;
